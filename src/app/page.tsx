...
// PLF Tables with CORRECT values from your tables
const HECM_PLF = {
...
export default function NextStepCRM() {
  ...
  const validateProgramEligibility = (age: number, programType: string): { eligible: boolean; message?: string } => {
    ...
    if ((programType === 'Equity Plus' || programType === 'Peak' || programType === 'LOC') && age < 55) {
      return { eligible: false, message: `${programType} requires minimum age 55. Current youngest borrower is ${age}.` }
    }
  ...
  const updateClient = async () => {
    try {
      // Format data to match GHL webhook format for consistency
      const updateData = {
        ...
        spouse_first_name: editingClient.spouse_first_name || null,
        spouse_last_name: editingClient.spouse_last_name || null,
        spouse_date_of_birth: editingClient.spouse_date_of_birth || null,
        home_value: editingClient.home_value || 0,
        address: editingClient.address || '',
        property_type: editingClient.property_type || 'Single Family Residence',
        mortgage_balance: editingClient.mortgage_balance || 0,
        occupancy_status: editingClient.occupancy_status || 'Primary Residence',
        program_type: editingClient.program_type || 'HECM',
        pipeline_status: editingClient.pipeline_status || 'Proposal Out',
